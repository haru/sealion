 ...
 * Using a Wednesday in the middle of March avoids edge cases at month/week boundaries.
 */
const FIXED_NOW = new Date("2026-03-25T14:30:00.000Z");
...
describe("buildDateWhere", () => {
  ...
  it('returns empty object for preset "none" on providerCreatedAt field', () => {
    const result = buildDateWhere("providerCreatedAt", "none");
    expect(result).toEqual({});
  });

  it('returns empty object for preset "none" on providerUpdatedAt field', () => {
    const result = buildDateWhere("providerUpdatedAt", "none");
    expect(result).toEqual({});
  });
});
