 ...
 *
 * @param data - The parsed response body (guaranteed to be a non-null object).
 * @returns The extracted message string, or null if no format matched.
 */
function extractFromObject(data: Record<string, unknown>): string | null {
  ...
  // Asana API BadRequest: { data: { error: "message" } }
  if ('data' in data) {
    const inner = data.data;
    if (typeof inner === 'object' && inner !== null && 'error' in (inner as Record<string, unknown>)) {
...
export function extractProviderMessage(error: unknown): string | undefined {
  ...
  // Provider-specific JSON formats (GitHub, Asana, Jira, etc.)
  if (typeof data === 'object' && data !== null) {
    const extracted = extractFromObject(data as Record<string, unknown>);
    if (extracted !== null) {
      return extracted;
    }
  }
